...
from utils.plots import color_list, plot_one_box, plot_with_ocr
...
class Detections:
            ...
            if pred is not None:
                ...
                if show or save or render:
                    for *box, conf, cls in pred:  # xyxy, confidence, class
                        ...
                        #plot_one_box(box, img, label=label, color=colors[int(cls) % 10]) # change this
                        self.num_plate[int(cls)] = plot_with_ocr(box, img, label=label, color=colors[int(cls) % 10])
    ...
    def pandas(self):
        ...
        ca = 'xmin', 'ymin', 'xmax', 'ymax', 'confidence', 'class', 'name', 'num_plate' # xyxy columns
        cb = 'xcenter', 'ycenter', 'width', 'height', 'confidence', 'class', 'name', 'num_plate' # xywh columns
        for k, c in zip(['xyxy', 'xyxyn', 'xywh', 'xywhn'], [ca, ca, cb, cb]):
            a = [[x[:5] + [int(x[5]), self.names[int(x[5])], self.num_plate[int(x[5])]]  for x in x.tolist()] for x in getattr(self, k)]  # update
