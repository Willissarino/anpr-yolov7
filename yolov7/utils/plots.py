...
paddle = PaddleOCR(lang="en", rec_algorithm='CRNN', show_log=False) # Load OCR
...
# TEST ZONE
def plot_with_ocr(x, img, color=None, label=None, line_thickness=3):
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    ...
    num_plate = paddle.ocr(sharpened, det=False, cls=False)
    flattened = list(itertools.chain.from_iterable(num_plate))
    num_plate, ocr_confidence = max(flattened, key=itemgetter(1), default=("", 0))
    
    # Remove special characters
    num_plate = re.sub(r'[^A-Z0-9-\s]', '', num_plate)
    ...
    if label:
        ...
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(img, num_plate, (c1[0], c1[1] - 2), 0, tl / 2, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
    return num_plate
